...
def create_and_populate(module: GemmaTextEncoder) -> GemmaTextEncoder:
    model = module.model
    # transformers ≥5.x: SiglipVisionModel no longer wraps a .vision_model child —
    # vision_tower IS the SiglipVisionModel (which has .embeddings directly).
    _vt = model.model.vision_tower
    v_model = getattr(_vt, "vision_model", _vt)
    l_model = model.model.language_model

    config = model.config.text_config
    dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)

    # transformers ≥5.x renamed rope_local_base_freq → rope_scaling is now a
    # nested dict: {sliding_attention: {rope_theta: ...}, full_attention: {rope_theta: ...}}
    # Older transformers had rope_local_base_freq as a flat attribute.
    if hasattr(config, "rope_local_base_freq"):
        local_base = config.rope_local_base_freq
    elif isinstance(config.rope_scaling, dict) and "sliding_attention" in config.rope_scaling:
        local_base = config.rope_scaling["sliding_attention"].get("rope_theta", 10000.0)
    else:
        local_base = 10000.0
    local_rope_freqs = 1.0 / (local_base ** (torch.arange(0, dim, 2, dtype=torch.int64).to(dtype=torch.float) / dim))

    # transformers ≥5.x removed "default" from ROPE_INIT_FUNCTIONS; full-attention
    # theta lives in rope_scaling["full_attention"]["rope_theta"].
    rope_scaling = config.rope_scaling
    if isinstance(rope_scaling, dict) and "rope_type" in rope_scaling:
        # old flat format: {"rope_type": "default", ...}
        rope_type = rope_scaling["rope_type"]
        if rope_type in ROPE_INIT_FUNCTIONS:
            inv_freqs, _ = ROPE_INIT_FUNCTIONS[rope_type](config)
        else:
            full_theta = rope_scaling.get("rope_theta", getattr(config, "rope_theta", 10000.0))
            inv_freqs = 1.0 / (full_theta ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))
    elif isinstance(rope_scaling, dict) and "full_attention" in rope_scaling:
        # new nested format
        full_theta = rope_scaling["full_attention"].get("rope_theta", 1000000.0)
        inv_freqs = 1.0 / (full_theta ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))
    else:
        full_theta = getattr(config, "rope_theta", 10000.0)
        inv_freqs = 1.0 / (full_theta ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim))

    # transformers ≥5.x: SiglipVisionModel exposes position_ids directly (no .embeddings child)
    v_emb = getattr(v_model, "embeddings", v_model)
    positions_length = len(v_emb.position_ids[0])
    position_ids = torch.arange(positions_length, dtype=torch.long, device="cpu").unsqueeze(0)
    v_emb.register_buffer("position_ids", position_ids)
    ...
    # transformers ≥5.x removed rotary_emb_local; register inv_freq only on what exists
